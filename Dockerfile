...
RUN \
    GO_ARCH=$(uname -m | sed -e 's/x86_64/amd64/' -e 's/\(arm\)\(64\)\?.*/\1\2/' -e 's/aarch64$/arm64/') && \
    wget -qO-                          "https://github.com/jkroepke/helm-secrets/releases/download/${HELM_SECRETS_VERSION}/secrets-${HELM_SECRETS_VERSION#v}.tgz" | tar -C /gitops-tools/helm-plugins -xzf- && \
    wget -qO-                          "https://github.com/jkroepke/helm-secrets/releases/download/${HELM_SECRETS_VERSION}/secrets-getter-${HELM_SECRETS_VERSION#v}.tgz" | tar -C /gitops-tools/helm-plugins -xzf- && \
